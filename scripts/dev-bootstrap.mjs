 ...
 * 5) turbo build (desktop) — electron(apps/desktop) main/preload/renderer 빌드
 *                            (turbo 캐시로 변경 없으면 즉시 통과)
 *
 * 이 스크립트는 root 의 `npm run predev` 에서 호출되며, 모든 단계가 끝나면 종료한다.
 * 이후 turbo run dev 가 web/api/storybook/desktop + Prisma Studio 를 동시 구동한다.
 *
 * 환경 변수:
...
async function main() {
  console.log("[dev-bootstrap] 1/5 docker compose up -d");
  const up = await run("docker", ["compose", "up", "-d"]);
  if (up.code !== 0) {
    throw new Error(`docker compose up 실패 (exit=${up.code})`);
  }

  console.log("[dev-bootstrap] 2/5 Postgres 준비 대기");
  ...
  console.log("[dev-bootstrap] 3/5 prisma migrate deploy");
  const migrate = await run("npx", ["prisma", "migrate", "deploy"]);
  if (migrate.code !== 0) {
    throw new Error(`prisma migrate deploy 실패 (exit=${migrate.code})`);
  }

  console.log("[dev-bootstrap] 4/5 prisma generate");
  const generate = await run("npx", ["prisma", "generate"]);
  if (generate.code !== 0) {
    throw new Error(`prisma generate 실패 (exit=${generate.code})`);
  }

  // electron(apps/desktop)은 main/preload/renderer가 빌드되어 있어야 dev 기동이 안정적이다.
  // 루트에서 turbo build 를 호출하며, 변경이 없으면 turbo 캐시로 즉시 통과한다.
  console.log("[dev-bootstrap] 5/5 apps/desktop(electron) 빌드");
  const desktopBuild = await run(
    "npx",
    ["turbo", "run", "build", "--filter=desktop"],
    { cwd: repoRoot },
  );
  if (desktopBuild.code !== 0) {
    throw new Error(`apps/desktop(electron) 빌드 실패 (exit=${desktopBuild.code})`);
  }
