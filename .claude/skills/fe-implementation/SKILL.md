...
작업 시작 전 `packages/shared-types`에 필요한 타입이 존재하는지 확인. 없으면 먼저 `packages/shared-types/src/`에 타입을 추가하고 `index.ts` 재수출 후 진행.
...
필요한 토큰이 없으면 먼저 `packages/ui/src/styles/globals.css`의 `@theme` 블록에 토큰을 추가하고 진행한다 (네임스페이스 매핑 규칙은 [.claude/rules/tailwind-v4.md](../../rules/tailwind-v4.md) 참조).
