...
신규 기능 키워드 감지 시 코드·테스트 변경에 들어가기 전 진입 게이트를 통과한다.
메인테이너 환경에서는 `harness-entry-gate` 룰의 4단계(slug 추출 → INDEX 조회 → 분기 판정 → 사용자 결정 5W1H 수집)를 적용한다.

1. 기능 slug를 추출한다. 불명확하면 사용자에게 kebab-case로 확인받는다.
2. 신규 기능이면 사용자 결정 항목(입력 자료 범위·라우팅/UI 옵션·비목표·호환성 영향)을 먼저 수집한다. 답을 받기 전까지 임시 결정으로 구현에 진입하지 않는다.
3. ccp CLI 준비 상태를 확인한다 (`/ccp:gemini-setup`, `/ccp:codex-setup`).
4. 산출물 기록이 필요하면 "산출물 기록" 단계(아래 참조)를 활성화한다.
